"""The four runtime prompts — SINGLE SOURCE (migration plan §3.4).

These four prompts are the ONLY prompts that ever enter a system prompt at
runtime: the supervisor prompt and the three curated skill prompts, copied
verbatim (byte-for-byte) from ``backend/app/services/agent_plugins/`` in the
source project.  ``skill/<name>/SKILL.md`` stays an archived resource and
never enters a system prompt.
...
module constants.  ``PROMPT_SOURCE`` records the file anchor each prompt was
ported from (``<file>:<start>-<end>``).
"""
...
PROMPT_SOURCE: dict[str, str] = {
    'supervisor': 'backend/app/services/agent_plugins/composition.py:337-353',
