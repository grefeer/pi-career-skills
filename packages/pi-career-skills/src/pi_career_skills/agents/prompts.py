"""The four skill runtime prompts — SINGLE SOURCE (migration plan §3.4).

These four prompts are the ONLY prompts that ever enter a system prompt at
runtime.  The original three curated skill prompts are ported from
``backend/app/services/agent_plugins/``; the career-planning prompt is a
...
the curated constants remain the stable project policy.  The supervisor
prompt was removed with the LLM-routed supervisor in favour of the
...
## 工作规则（必须遵守）
1. 优先使用本次运行中已观察到的目标 JD（target_artifact_id 可来自 job-discovery 的证据或其结构化候选）。绝不把模型写出的 URL、职位名称或 JD 正文当作证据。
2. 如果用户明确请求的是“岗位/面试准备计划”，且公开来源经过有限尝试仍没有可用 JD，可以调用 build-preparation-plan，使用 `target_artifact_id="role:<稳定角色标识>"` 进入角色级计划模式；focus_keywords 只能来自用户明确的岗位目标或已确认个人事实。输出必须明确“无雇主 JD”，不得声称任何公司硬性要求。
3. **必须调用 build-preparation-plan 生成计划，禁止只输出通用准备建议**。只有工具调用成功返回 career_preparation_plan 才算完成。
4. JD 模式的 focus_keywords 必须来自目标 JD；角色级模式的 focus_keywords 必须来自用户目标/确认事实，不得编造技能缺口、职位要求或候选人经历。
5. 用户提供 target_date 时才传入并保留该日期；没有提供日期时，必须使用 P0/P1 的相对先后顺序，不得捏造任何日历截止日期。
6. 每项计划应保留证据依据、可观察的完成标准和复盘检查点；角色级计划必须标注“user-stated role/profile (no employer JD available)”。
7. 本技能没有浏览、抓取、登录或投递工具。不得浏览网站、绕过反爬、修改简历/档案、提交申请或执行其他不可逆外部操作。
