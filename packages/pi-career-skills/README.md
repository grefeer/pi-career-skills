...
Pi-py 移植的求职多 Agent 评测运行时：三个技能（job-discovery / job-matching /
resume-tailoring）、12 个确定性工具、以及 run 级 harness（证据库、完成门、预算、
stall、有界自动恢复）。

> 状态：评测/对齐运行时（内存态 run 级状态），不是生产后端。迁移实施计划见源项目
> `docs/pi-py-agent迁移计划.md`；12 工具契约快照见源项目 `docs/pi_contract_snapshot.json`。
...
├── errors.py           # 稳定错误码与脱敏
├── registry.py         # 12 ToolDefinition（唯一真值）
